...
def tambah_pengguna(username, password, role='user'):
    global users  
    if username not in users:
        users[username] = {"password": password, "role": role}
        print(f"Pengguna '{username}' berhasil ditambahkan.")
    else:
        print(f"Pengguna '{username}' sudah ada.")


def tampilkan_event_recursively(events, index=0):
    ...
    tampilkan_event_recursively(events, index + 1)


def hapus_event_pengguna(nama_event):
    global jadwal_event  
...
def tampilkan_semua_event():
    global nama_event_list  
    
    print("--- Event yang Sudah Ada di Sistem ---")
...
while True:
    ...
    if pilihan == '1':
        ...
        tambah_pengguna(username, password)  
        ...
        if user_login["role"] == "admin":
            while True:
                ...
                if admin_pilihan == '1':
                    ...
                    hapus_event_pengguna(nama_event)  
            ...
            while True:
                ...
                if user_pilihan == '1':
                    ...
                    if username in jadwal_event and jadwal_event[username]:
                        tampilkan_event_recursively(jadwal_event[username])  
                    else:
